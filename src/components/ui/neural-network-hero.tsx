...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[5]
    ...
    // layer 9 - Modified for RED/BLACK/WHITE palette ********************************************************************
    ...
    * buf[7]
    ...
    // Apply red/black/white color transformation
    float r = buf[0].x * 0.94 + 0.06; // Boost red channel (#ef3435 = 0.94, 0.20, 0.21)
    float g = buf[0].y * 0.2;
    float b = buf[0].z * 0.21;
...
export default function NeuralNetworkHero({
  title,
  description,
  badgeText = "Visión Perfecta",
  badgeLabel = "Shamir",
  ctaButtons = [
    { text: "Agenda tu evaluación", href: "#contacto", primary: true },
    { text: "Descubre la gama", href: "#productos" }
  ],
  microDetails = ["Tecnología Freeform", "IA Visual", "Personalización Total"]
}: HeroProps) {
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-full overflow-hidden">
      ...
      <div className={`relative z-10 mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <h1 className="max-w-3xl text-left text-4xl font-extralight leading-[1.05] tracking-tight text-white sm:text-5xl md:text-6xl lg:text-7xl">
          {title}
        </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border px-5 py-3 text-sm font-medium tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30 duration-300 ${
                button.primary
